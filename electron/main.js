...
function waitForBackend(port, maxAttempts = 30) {
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      attempts++;
      const req = http.request({ host: 'localhost', port, path: '/api/health', timeout: 500 }, (res) => {
        resolve();
      });
...
function createWindow() {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    const indexPath = path.join(appPath, 'frontend/dist/mi-app-frontend/browser/index.html');
